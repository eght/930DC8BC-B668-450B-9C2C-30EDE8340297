...
% See http://people.delphi.com/rickadams/adventure/ for related info.
...
\datethis
@* Introduction. The ur-game for computers --- Adventure --- was
originally written by Will Crowther in 1975 and greatly extended by
@^Crowther, William R.@>
Don Woods in 1976. I have taken Woods's original {\mc FORTRAN} program
@^Woods, Donald Roy@>
...
@ The |action| words tell us to do something that is usually nontrivial.
...
rocky bed.",@/
"You're in valley.",lighted+liquid); @q +liquid added to original program @>
...
back, declaring, \"Good workmanship, but it's not valuable enough.\n");
 else if (here(BEAR) && prop[BEAR]) @<Throw the axe at the bear@>@;
 else {
...
@<Make the pirate track you@>=
{
  if (loc!=max_pirate_loc && prop[CHEST]<0) {
    for (i=min_treasure,k=0;i<=max_obj;i++) if (!too_easy(i)) {
      if (toting(i)) @<Take booty and hide it in the chest@>;
      if (place[i]==loc) k=1; /* a treasure with |base[i]!=NOTHING| */
    }
...
@ After the cave is closed, we look for objects being toted with |prop<0|;
